...
// Consistent hydration-safe date formatter
function formatReservationDate(dateInput: string | Date): string {
  if (!dateInput) return '';
              ...
              {sortedFiltered.length === 0 ? (
                ...
                sortedFiltered.map((res) => (
                  <tr key={res.id} className="hover:bg-zinc-50 transition-colors">
                    ...
                    <td className="p-5">
                      <div className="flex items-center gap-1 font-semibold text-zinc-800" suppressHydrationWarning>
                        <Calendar size={12} className="text-zinc-400" />
                        <span>{formatReservationDate(res.date)}</span>
                      </div>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <a
                          href={`https://wa.me/${res.phone.replace(/\D/g, '').startsWith('91') || res.phone.replace(/\D/g, '').length > 10 ? '' : '91'}${res.phone.replace(/\D/g, '')}?text=${encodeURIComponent(
                            `Hello ${res.customerName || 'Customer'}, regarding your table reservation Ref: ${res.bookingRef} on ${formatReservationDate(res.date)} at ${res.time} at Balaji Chilkur Family Dhaba:`
                          )}`}
                          target="_blank"
